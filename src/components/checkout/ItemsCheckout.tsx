    ...
    <div>
        ...
        {cartItems.map((item) => (
          ...
          >
            <div className="flex relative border border-stone-300 bg-stone-200 rounded-md">
              ...
              />
              <span className="w-5 h-5 rounded-full bg-gray-500 text-white flex items-center justify-center text-xs absolute -right-2 -top-2 font-medium">
                {item.quantity}
              </span>
            </div>
      ...
      <div className="mt-4 p-7 space-y-5">
        ...
        <div className="flex justify-between font-semibold text-lg ">
          <p className="">Total:</p>
          <p className="">{formatPrice(totalAmount)}</p>
        </div>
      </div>
    </div>
  );
