...
export const createOrder = async (order: OrderInput) => {
  // 1 obtener el usuario auntenticado + cliente de la tabla customer
  ...
  for (const item of order.cartItems) {
    ...
    if (variantData.stock < item.quantity) {
      throw new Error(
        `No hay suficiente stock para los articulos seleccionados `
      );
    }
  }
  ...
  for (const item of order.cartItems) {
    ...
    if (!variantData) {
      throw new Error("Error al obtener el stock de la variante");
    }
    ...
    if (updatedStockError) {
      ...
      throw new Error("Error al actualizar el stock de la variante");
    }
  }
